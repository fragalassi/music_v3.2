...
from CNN_training_tools.build_model import define_training_layers, fit_model
...
def train_cascaded_model(model, train_x_data, train_y_data, options):
    ...
    if options['full_train'] is False:
        ...
        print( '> DEBUG: opt patience = ',options['patience'])
        options['max_epochs'] = 0
        for it in range(0, max_epochs, 10):
            print( '> DEBUG: patience = ',patience)
            print( '> DEBUG: current val_loss = ',best_val_loss)
...
#        options['patience'] = 5
    ...
    if options['full_train'] is False:
        ...
        for it in range(0, max_epochs, 10):
            ...
            # evaluate if continuing training or not
            val_loss = min(model[1]['history'].history['val_loss'])
            if val_loss > best_val_loss:
                patience += 10
            else:
                best_val_loss = val_loss
...
#    modalities = train_x_data[scans[0]].keys()
    modalities = train_x_data[list(scans)[0]]#ERROR
...
def load_test_patches(test_x_data,
    ...
    modalities = (test_x_data[list(scans)[0]])
    #print (modalities)
...
def get_patches(image, centers, patch_size=(15, 15, 15)):
    ...
    # it will try to create a square almost centered. By doing this we allow
    ...
    if list_of_tuples and sizes_match:
        patch_half = tuple([idx//2 for idx in patch_size]) #changed / to // to get integer in python 3
...
def test_scan(model,
    ...
    if options['debug'] is True:
            #print( "> DEBUG ", list(scans[0]), "Voxels to classify:", all_voxels)
    ...
    if check_min_error(seg_image, options, flair_image.header.get_zooms()):
        if options['debug']:
            print( "> DEBUG... ")
            #print( "> DEBUG ", list(scans[0]), "lesion volume below ", \
