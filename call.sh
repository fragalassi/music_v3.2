...
for patient in $DATADIR/*; do	
	...
	PYTHONHASHSEED=0 python3 animaMSExamPreparation.py -r $FLAIR -f $FLAIR -t $T1  -o $DATADIR/"$patientID"/ 
	...
	PYTHONHASHSEED=0 python3 animaMusicLesionSegmentation_v3.py -f $FLAIR -t $T1 -m $MASK -n 6 -o $DATADIR/"$patientID"/
